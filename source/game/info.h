...
struct Block {
  ...
  static void Parse(Res::UMF umf) {
    ...
    for (auto& field : umf) {
      std::pair<string, string> idTitle = Utils::Split(field.first, '/');
      ...
      if (idTitle.second == "name")
        ids[sval] = id, blocks[id].name = sval;

      else if (idTitle.second == "charinfo")
        blocks[id].character = sval[0], blocks[id].fg = Utils::Chtoi(sval[1]),
        blocks[id].bg = Utils::Chtoi(sval[2]);

      else if (idTitle.second == "collision")
        blocks[id].entCol = sval[0] == '1', blocks[id].projCol = sval[1] == '1';
...
struct Entity {
  ...
  static void Parse(Res::UMF umf) {
    ...
    for (auto& field : umf) {
      ...
      if (idTitle.second == "name")
        ids[sval] = id, entities[id].name = sval;
...
struct Item {
  ...
  static void Parse(Res::UMF umf) {
    ...
    for (auto& field : umf) {
      ...
      if (idTitle.second == "name")
        ids[sval] = id, items[id].name = sval;
