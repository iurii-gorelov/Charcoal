...
class Area {
 ...
 public:
  ...
  v2s ToWorld(v2s pos) {
    return pos + camera - v2s(cl::Width() / 2, cl::Height() / 2);
  }
  v2s ToScreen(v2s pos) {
    return pos - camera + v2s(cl::Width() / 2, cl::Height() / 2);
  }
  ...
  void BlockPut(int id, int x, int y) {
    if (!Inside(v2s(x, y))) return;
    auto& info = Info::blocks[id];
    auto& block = blocks[x + y * size.x];
    if (info.light > 0) LightCreate(v2s(x, y), info.light), block.ex = 'l';
    block.id = id;
  ...
  void BlockRender(Block& block, v2s spos, v2s wpos) {
    ...
    if (info.name == "water") {
      ...
      if (ut::ticks / 4 % 8 == (wpos.x + wpos.y + 9999) % 8) std::swap(fg, bg);
      cl::Put(info.character, spos.x, spos.y, fg, bg);
    }

    else if (info.name == "fireplace")
      cl::Put(info.character, spos.x, spos.y, ut::ticks / 6 % 2 ? 0xc : 0x4,
              0xff);

    else
      cl::Put(info.character, spos.x, spos.y, info.fg < 16 ? info.fg : 0xff,
  ...
  void BlockDamage(v2s pos, int damage) {
    ...
    if (block.hp == 0) {
      if (info.drop != "") EntityAdd(new Drop(this, pos, info.drop));
      if (info.light > 0) LightRemove(pos);
      block.id = 0;
    }
  }
  ...
  void LightRemove(v2s pos) {
    for (int i = 0; i < lights.size(); i++)
      if (lights[i]->pos == pos) lights.erase(lights.begin() + i);
  }

  void LightRemove(Light* light) {
    for (int i = 0; i < lights.size(); i++)
      if (lights[i].get() == light) lights.erase(lights.begin() + i);
  }
  ...
  float LightLevel(v2s pos) {
    ...
    for (auto& light : lights) {
      ...
      if (sqdist > lightsq) continue;

      result += 1 - sqdist / lightsq;

      if (result > 0.7) return result;
    }
    return result;
  }

  void EntityAdd(Entity* entity) { entities.push_back(uptr<Entity>(entity)); }
  ...
  void Update(void) {
    for (int i = 0; i < entities.size(); i++) entities[i]->Behave();
    ...
    if (daytime < TIME_DAWN)
      ...
      lightScalar =
          2 + 5 * (1 - scast<float>(daytime - DAY_LENGTH / 2) / TIME_DAWN);

    for (int i = 0; i < RANDOM_PING_COUNT; i++) RandomPing();

    time++;

    if (time == DAY_LENGTH) Scene::Switch("YouWin");
  }
  ...
  void Render(void) {
    ...
    for (auto& entity : entities)
      entity->Render(entity->pos.x - camera.x + cl::Width() / 2,
    ...
    for (int x = 0; x < cl::Width(); x++)
      for (int y = 0; y < cl::Height(); y++)
        lightMask[x + y * cl::Width()] = LightLevel(v2s(
            x + camera.x - cl::Width() / 2, y + camera.y - cl::Height() / 2));
  ...
  void RandomPing(void) {
    ...
    if (Inside(v2s(x, y)) &&
        (Info::blocks[blocks[x + y * size.x].id].entCol ||
         Info::blocks[blocks[x + y * size.x].id].projCol)) {
      auto& block = blocks[x + y * size.x];
      auto& info = Info::blocks[block.id];
      if (info.name == "wheatseeds")
        block.id = Info::Block::ids["wheat"];
      else if (info.name == "sapling")
        block.id = rand.Chance(0.1) ? Info::Block::ids["redtree"]
                                    : Info::Block::ids["tree"];
      return;
    }
