...
void Entity::Death(void) { area->EntityRemove(this); }

void Entity::TryMove(int dirX, int dirY) {
  if (pos.x + dirX < 0 || pos.x + dirX >= area->size.x || pos.y + dirY < 0 ||
      pos.y + dirY >= area->size.y)
    return;

  v2s tarPos = pos + v2s(dirX, dirY);

  Block& block = area->blocks[tarPos.x + (tarPos.y) * area->size.x];
  if (Info::blocks[block.id].entCol) return;

  for (auto& entity : area->entities)
    if (Info::entities[entity->id].collidable && entity->pos == tarPos) return;
...
Player::Player(Area* area, v2s pos)
    : Entity(area, pos, Info::Entity::ids["player"]) {
...
void Player::Behave(void) {
  ...
  if (cl::Pressed('X')) {
    ...
    if (dirX != 0 || dirY != 0) UseItem(v2s(dirX, dirY));
  }
...
void Player::SelectItem(int slot) {
  ...
  if (Game::selectedSlot >= Game::inventory.size()) Game::selectedSlot = 0;
...
void Player::UseItem(v2s dir) {
  if (!useTimer.Check(false)) return;
  ...
  if (item.type == 2) {
    ...
    if (Info::blocks[slot.id].entCol || Info::blocks[slot.id].projCol) return;
    area->BlockPut(Info::Block::ids[Info::items[itemId].name], targetPos.x,
                   targetPos.y);
...
void Projectile::Behave(void) {
  if (area->Inside(pos) &&
      Info::blocks[area->blocks[pos.x + pos.y * area->size.x].id].projCol) {
    if (players)
  ...
  for (auto& entity : area->entities) {
    if (entity->pos == pos && Info::entities[entity->id].collidable &&
        entity.get() != owner) {
...
void Drop::Behave(void) {
  if (pos == area->player->pos) {
    for (int i = 0; i < drops.size(); i++) Game::AddItem(drops[i]);
    area->EntityRemove(this);
  }
}
...
Projectile::Projectile(Area* area, v2s pos, v2s dir, Entity* own,
                       Info::Item& wand)
...
void Enemy::Behave(void) {
  int dist = 64 * 64;
  if (area->time < DAY_LENGTH / 2) dist = 32 * 32;
  ...
  if (moveTimer.Check()) {
    if (info.name == "demon" || info.name == "priest") {
      TryMove(dir.x, 0);
      TryMove(0, dir.y);
      if (Info::blocks[area->blocks[pos.x + pos.y * area->size.x].id].name ==
          "peaks")
        ApplyDamage(2);
      moveTimer.Reset();
    }
  ...
  if (attackTimer.Check()) {
    ...
    if (info.name == "priest") {
      area->EntityAdd(new Projectile(
          area, pos, dir, this, Info::items[Info::Item::ids["priestwand"]]));
      attackTimer.Reset();
    }
  }
}
