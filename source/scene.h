...
#define SC_DEFAULT_INIT \
  static void Init(void) {}
#define SC_DEFAULT_UPDATE \
...
class Scene {
 ...
 public:
  ...
  static void InitAll() {
    for (auto& scene : Scenes()) scene.second->init();
  }

  static void UpdateCurrent() { sceneptr->update(); }

  static void Switch(string name) { sceneptr = Scenes()[name]; }
};