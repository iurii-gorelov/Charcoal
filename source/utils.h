...
struct v2s {
  ...
  v2s Normalize(void) {
    return v2s(x == 0 ? 0 : x > 0 ? 1 : -1, y == 0 ? 0 : y > 0 ? 1 : -1);
  }

  v2s operator+(v2s other) { return v2s(x + other.x, y + other.y); }
  v2s operator-(v2s other) { return v2s(x - other.x, y - other.y); }
  v2s operator*(int scalar) { return v2s(x * scalar, y * scalar); }

  bool operator==(v2s other) { return x == other.x && y == other.y; }
};
...
class Random {
 public:
  ...
  int Next() { return ((seed = (seed * 214013 + 2531011)) & 0xfffff) >> 4; }

  int Next(int max) { return Next() % max; }

  int Next(int min, int max) { return min + Next() % (max - min); }

  bool Chance(float chance) { return Next() / scast<float>(0xffff) < chance; }
