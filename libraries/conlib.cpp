...
void ConLib::Title(const char* title) { SetConsoleTitleA(title); }

bool ConLib::FullScreen(void) { return isFullScreen; }
...
void ConLib::FullScreen(bool enable) {
  ...
  if (enable && !isFullScreen) {
    ...
    SetWindowPos(window, HWND_TOPMOST, 0, 0, screenWidth, screenHeight,
    ...
    SetWindowPos(window, NULL, windowRect.left, windowRect.top,
                 windowRect.right - windowRect.left,
                 windowRect.bottom - windowRect.top, SWP_FRAMECHANGED);
...
int ConLib::Width(void) { return bufsize.X; }

int ConLib::Height(void) { return bufsize.Y; }
...
void ConLib::InstallFont(const char* path) {
  ...
  std::string dest = std::string(localAppData) +
                     "\\Microsoft\\Windows\\Fonts\\" + name + ".ttf";
  ...
  RegSetValueExA(hKey, name.c_str(), 0, REG_SZ, (BYTE*)dest.c_str(),
                 dest.length() + 1);
  RegCloseKey(hKey);
}

void ConLib::Put(unsigned char c, int x, int y, unsigned char fg,
                 unsigned char bg) {
  if (x < 0 || x >= bufsize.X || y < 0 || y >= bufsize.Y) return;
  buffer[y * bufsize.X + x].Char.AsciiChar = c;

  WORD& attr = buffer[y * bufsize.X + x].Attributes;
  if (fg < 16) attr = attr & 0xf0 | fg;
  if (bg < 16) attr = attr & 0x0f | (bg << 4);
}
...
void ConLib::ColorTable(int table[16]) {
  ...
  if (noScrollBar) ConLib::NoScrollBar();
}
...
void ConLib::Sleep(int ms) { ::Sleep(ms); }

bool ConLib::Pressed(int key) { return GetAsyncKeyState(key) < 0; }
...
void ConLib::Write(std::string text, int x, int y, int maxw) {
  ...
  for (int i = 0, di = 0; i < text.length(); i++) {
    if (text[i] == '\f')
      i++,
          fg = text[i] >= '0' && text[i] <= '9' ? text[i] - '0' : text[i] - 'W';
    else if (text[i] == '\b')
      i++,
          bg = text[i] >= '0' && text[i] <= '9' ? text[i] - '0' : text[i] - 'W';
    else
      ConLib::Put(text[i], x + (di % maxw), y + (di / maxw), fg, bg), di++;
  }
}

void ConLib::Fill(unsigned char c, int x, int y, int w, int h, unsigned char fg,
                  unsigned char bg) {
  for (int i = 0; i < w; i++)
    for (int j = 0; j < h; j++) ConLib::Put(c, x + i, y + j, fg, bg);
}